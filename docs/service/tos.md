...
c30（ced）が運営する分散型マイクロブログSNS Misskeyの非公式フォーク「misskey-juice」の公式インスタンス「Juice Server」（以下「本サービス」といいます）をご利用いただく際は、下記ルールを基準にマナーを守ってご利用いただくようお願いします。
...
- 本サービスは投稿を行った瞬間に連合している世界中のインスタンスに送信と複製が行われる「ActivityPub」という仕組み利用した分散型SNSであり、一度投稿した情報は完全に削除するのが困難となる可能性があります。
- 本サービスは日本国内向けに提供されているサービスであり、利用規約等の準拠法は日本法とします。
- 個人情報保護方針は日本国の個人情報保護法に基づいたものとします。
- 児童への安全性とプライバシー保護の観点から13歳未満による本サービスの利用及び
  未成年者による本名や顔写真など個人を特定できる情報をインスタンスに送信する行為を禁止します。
- 本サービスの内容上、16歳未満の方の登録はご遠慮いただいております。
- 当方は本サービスで発生したユーザー間のトラブルや不利益についていかなる場合も責任を負いません。
...
- 児童を性的に描写したコンテンツの投稿(実写・二次元(イラスト等)を問いません)
- 13歳未満による本サービスの利用
- 未成年者による本名や顔写真など個人を特定できる情報をインスタンスに送信する行為
...
「内容を隠す(CW)」やNSFWフラグを設定していないNSFW投稿(画像・文章・MFMによる文字装飾やアスキーアート・リノートを含む)は、投稿の削除やNSFWフラグの付与に加えて、違反を繰り返した回数に応じて段階的にアカウントへの処罰を行います(1回目:注意のみ、2回目:サイレンス、3回目:凍結)。詳しくは[ルール](./rules.md#nsfw-not-safe-for-work-に関して)をご確認ください。
...
本サービスはActivityPubという仕組みを利用した分散型SNSであり、投稿やプロフィールなどの情報は連合しているインスタンスへ自動的に送信・複製されます。そのため、本サービス上でアカウントや投稿を削除しても、その削除は連合先の他インスタンスへ即座に反映されるとは限らず、削除依頼への対応方針が異なるインスタンスやすでに運用を終了したインスタンスなどでは、複製されたデータがそのまま残り続ける可能性があります。

アカウントの閉鎖（退会）や個々の投稿・ファイルの削除は、本サービスの機能を通じてご自身で行うことができます。ただし上記の性質上、当方が把握・管理できない外部インスタンスに残留したデータについては削除を保証できず、一切の責任を負わないものとします。
