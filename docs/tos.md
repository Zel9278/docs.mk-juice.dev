...
c30（ced）が運営する分散型マイクロブログSNS Misskeyの非公式フォーク「misskey-juice」の公式サーバー「Juice Server」（以下「本サービス」といいます）をご利用いただく際は、下記ルールを基準にマナーを守ってご利用いただくようお願いします。
...
本サービスはActivityPubという仕組みを利用した分散型SNSであり、投稿やプロフィールなどの情報は連合しているサーバーへ自動的に送信・複製されます。そのため、本サービス上でアカウントや投稿を削除しても、その削除は連合先の他インスタンスへ即座に反映されるとは限らず、削除依頼への対応方針が異なるサーバーやすでに運用を終了したサーバーなどでは、複製されたデータがそのまま残り続ける可能性があります。

アカウントやデータの削除を依頼する場合は本サービスのシステムか本サービスに登録しているEメールアドレスからのみ受け付けるものとします。ただし上記の性質上、当方が把握・管理できない外部インスタンスに残留したデータについては削除を保証できず、一切の責任を負わないものとします。
