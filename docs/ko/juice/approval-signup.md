...
"승인제 신규 가입"은 신규 가입 시 가입 사유 입력을 필수로 하고, 관리자가 내용을 확인한 후 승인하는 JUICE 고유 기능입니다. **Juice Server에서는 현재 이 기능을 사용하고 있지 않으므로, 누구나 가입 즉시 계정을 사용할 수 있습니다.** 아래는 이 기능이 활성화되어 있는 경우(자체 호스팅 인스턴스 등)의 흐름을 설명합니다.
...
flowchart TD
    ...
    C --> D{관리자가 심사}
    D -->|승인| E{이메일 등록・<br/>인증 완료?}
    E -->|예| E1["`로그인 가능<br/>**승인 알림 이메일 도착**`"]
    E -->|아니오| E2["`로그인 가능<br/>**확인 코드로 승인 확인**`"]
    D -->|반려| F{이메일 등록・<br/>인증 완료?}
    F -->|예| F1["`반려 알림 이메일 도착<br/>**다시 신규 가입 필요**`"]
    F -->|아니오| F2["`확인 코드로 반려 확인<br/>**다시 신규 가입 필요**`"]
```
...
- **승인된 경우**: 로그인할 수 있게 됩니다. 이메일 주소를 등록・인증한 경우 승인 알림 이메일도 발송되지만, 등록하지 않은 경우에는 발송되지 않으므로 `/signup-check` 페이지에서 확인 코드로 승인 상태를 확인해 주세요.
- **반려된 경우**: 이메일 주소를 등록・인증한 경우 반려 알림 이메일이 발송됩니다. 등록하지 않은 경우에도 마찬가지로 `/signup-check` 페이지에서 확인 코드로 반려 사실과 사유를 확인할 수 있습니다. 다시 신청하고 싶은 경우 새 계정으로 다시 가입해 주세요(반려된 신청 자체는 재개할 수 없습니다).
