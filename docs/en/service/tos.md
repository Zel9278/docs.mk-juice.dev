...
The following acts will result in silencing:
...
Bots that fall under the following categories require the bot developer to take countermeasures.
Also, all bot admins need to regularly check what content their bot is posting.

If the following countermeasures are not taken and the bot is abused, the bot account may be suspended.

- Bots that use search engines etc. to automatically post images based on keywords, etc.
  - Please filter and set NSFW so as not to display inappropriate images
- Bots that have the ability to reply or post publicly through user operations
  - Please set up rate limits so that a large number of posts are not made
  - Please filter so it does not respond to inappropriate words

## About collecting information

For the purpose of operating the service, this service collects the following information:

- Email address (if set)
- IP address
- Other information described in the [Privacy Policy](./privacy_policy.md)

The information collected is used only for the following purposes, and not for any other purpose:

- To provide this service, such as the email notification feature
- To warn users who violate the Terms of Service or to suspend accounts
- To improve the service experience

We will not disclose this information to third parties, except when required by law.
We also will not entrust the handling of this information to outside parties.
