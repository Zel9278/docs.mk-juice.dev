...
>
> なお、misskey-juiceは開発中のMisskeyフォークです。不具合などによりデータが失われる可能性はゼロではありません。あらかじめご了承ください。
...
デフォルトの全文検索(`sqlLike`)からpgroongaに切り替えることで、検索速度と精度(特に日本語などのCJK言語)が大きく向上します。導入方法・有効化の手順は、[0から構築するガイドのpgroongaセクション](./install.md#pgroongaのセットアップ-推奨)を参照してください。
